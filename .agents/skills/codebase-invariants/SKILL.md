...
Read these reference files BEFORE opening target source files:
...
- [references/dnd-rules-map.md](references/dnd-rules-map.md) — the 2014 5e rules→app math, render-time application order, the DND-* mechanics invariants, and the catalog of rules the app currently gets wrong. Read this whenever the change concerns a stat's **value** / rules correctness (AC, HP, spell DC, proficiency, weapon/skill/save math, resources). Full prose + citations live in [`DND_RULES_REFERENCE.md`](../../../docs/reference/DND_RULES_REFERENCE.md).
...
1. Load all three reference files, then read the target slice **whole** (the full
