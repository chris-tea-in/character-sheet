...
| # | Item | Status |
|---|---|---|
| C1 | **Fighting-style combat math** — **Great Weapon Fighting** (reroll 1s/2s on damage) and **Two-Weapon Fighting** (offhand ability mod to damage). Archery/Dueling/Defense ride `featureWeaponEffects` as flat per-weapon bonuses; GWF (a roll-time dice *reroll*, not a flat number) and TWF (needs an offhand-attack model the app doesn't have — and the app already adds the ability mod to every weapon's damage) don't fit that path. **Decision (2026-06-22): leave to manual play.** Both are already selectable in the Features block with full rules text shown ([FeaturesBlock.tsx](src/components/sheet/FeaturesBlock.tsx) → `DetailPopup`) and carry no `effects`, so nothing is auto-applied and no UI over-claims (INV-5). Same policy as Sharpshooter/GWM/Bless/Rage in *Render-Time Stat Pipeline Extensions* below. | ✅ Out of scope — manual play |
