import type { Race, ClassData, SubclassData, Background, FeatData, SpellData, EquipmentData, ClassFeatureData } from '@/types/data'
...
export function loadSetupData(): Promise<SetupData> {
  return setupCache ??= memoize(
    ...
    ]).then(([races, classes, subclasses, backgrounds, classFeatures]) => ({ races, classes, subclasses, backgrounds, classFeatures })),
    p => { setupCache = p },
  )
}
