...
| 3 | 🔴 | data | Class/Subclass Features & Resource Pools | Rage uses and Rage damage entirely untracked (Barbarian core resource) | public/data/class-features.json (no barbarian rage group); src/components/sheet/FeaturesBlock.tsx:208 (resource tracker only renders when group.resource exists) | RESOLVED 2026-07-04 |
| 4 | 🔴 | data | Class/Subclass Features & Resource Pools | Ki, Sorcery Points, Bardic Inspiration, Channel Divinity, Wild Shape, Action Surge, Lay on Hands have no resource tracker | src/lib/classFeatures.ts:52-63 (resourceCount returns 0 when group has no resource); public/data/class-features.json (only 2 groups carry resource) | PARTIAL 2026-07-04 |
...
- **RESOLVED (2026-07-04, `feat/class-abilities`):** Rage uses are tracked via the new class-abilities channel (`data/class-abilities.json` → ClassAbilitiesSection in the Spells area): by-level table 2/3/4/5/6 at 1/3/6/12/17, long-rest tag, bonus-action badge, usage persisted in `featureResourcesUsed` under `ability:barbarian:rage`. Rage *damage* stays a manual note per this finding's own "Correct handling" (situational, only while raging); L20 unlimited rages is likewise unrepresented (tracker caps at 6).
...
- **PARTIAL (2026-07-04, `feat/class-abilities`):** a dedicated registry now exists — `data/class-abilities.json` → `resolveResourceMax`/`earnedAbilities` (`src/lib/classFeatures.ts`) → ClassAbilitiesSection in the Spells area, sized by the OWNING class's level (INV-2) with usage in `featureResourcesUsed` (`ability:…` keys). **Tracked:** Lay on Hands (5×level pool), Second Wind, Action Surge (1 → 2 at 17), Channel Divinity (cleric 1/2/3 at 2/6/18; paladin 1 at 3), Bardic Inspiration (CHA mod, min 1), Ki (= monk level, with Flurry of Blows / Patient Defense / Step of the Wind spending it), Wild Shape (2/short). **Coverage extended 2026-07-05 (data-only, 40 entries):** all base-class counted resources are now tracked (adds Divine Sense (1+CHA via the `plus` sizing), Cleansing Touch, Divine Intervention, Sorcery Points + Tides of Chaos, Indomitable, Wholeness of Body + Stunning Strike/Empty Body (ki-costed), Stroke of Luck, Eldritch Master, Arcane Recovery, Flash of Genius, Hexblade's Curse, Portent, Blood Maledict), plus informational action-typed rows with no counter for on-hit/at-will features (Divine Smite, Sneak Attack, Reckless Attack, Cunning Action, Uncanny Dodge, Deflect Missiles, Slow Fall, Countercharm, Song of Rest, Primeval Awareness, Flexible Casting, Crimson Rite). Remaining gaps are niche subclass actives — pure data authoring. Rest refresh remains manual (tap pips / Reset) — the app has no rest system.
- **Review addendum (2026-07-04, adversarially confirmed):** the resource schema is deliberately static, which leaves three representation gaps — (1) Bardic Inspiration keeps its `long rest` tag past bard 5 (Font of Inspiration's short-rest recharge isn't expressible; the authored description text carries the rule); (2) Rage caps at 6 uses at barbarian 20 (RAW: unlimited — noted in #3); (3) Wild Shape caps at 2 uses at druid 20 (RAW: Archdruid grants unlimited, and the app's own Archdruid description says so). All three are informational-display limits, not spend-logic bugs; fixing them needs an `unlimited`/level-stepped-rest extension to the resource shape.
